...
import { channels, ideas, posts, thoughts } from './content.js';
...
function Feed({ t, embedded = false }) {
...
function Channels({ t, lang }) {
  return <section className="section page-hero reveal visible">
    <div className="section-heading"><p className="eyebrow">{t.channelsEyebrow}</p><h1>{t.channelsTitle}</h1><p className="lead">{t.channelsLead}</p></div>
    <div className="cards channel-grid">{channels.map(channel => <a className="card channel-card" key={channel.id} href={channel.url} target="_blank" rel="noreferrer"><span className="tag">{channel.type}</span><h3>{channel.title}</h3><p>{lang === 'ru' ? channel.descriptionRu : channel.descriptionEn}</p><strong>{t.open}</strong></a>)}</div>
  </section>;
}
